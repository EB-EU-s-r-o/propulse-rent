    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3">
          <span className="text-xs font-medium bg-background/80 backdrop-blur-sm text-foreground px-2.5 py-1 rounded-none border border-border/50">
            {property.type}
          </span>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mt-4">
          {property.features.slice(0, 3).map((feature) => (
            <span
              key={feature}
              className="text-[10px] font-medium text-muted-foreground bg-secondary px-2 py-0.5 rounded-none"
            >
              {feature}
            </span>
          ))}
          {property.features.length > 3 && (
            <span className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-none">
              +{property.features.length - 3}
            </span>
          )}
        </div>
...
const ActionButton = ({
  ...
  return (
    <button
      className={`p-2.5 rounded-none transition-all ${variants[variant]}`}
      title={label}
    >
      {icon}
    </button>
...
const OccupancyIcon = ({ percent }: { percent: number }) => {
  ...
  useEffect(() => {
    if (barRef.current) {
      barRef.current.style.setProperty('--height', `${percent}%`);
    }
  }, [percent]);

  return (
    <div className="w-3.5 h-3.5 rounded-none border-2 border-current relative overflow-hidden">
      <div
        ref={barRef}
