...
import {
    ...
    LogOut, Moon, Activity, ChevronRight
} from 'lucide-react';
...
export const UserDropdown = ({ isOpen, onClose }: UserDropdownProps) => {
    ...
    const role = roles.length ? roles.join(', ') : 'No role assigned';
    ...
    return (
        <div className="absolute top-16 right-6 w-80 bg-card/80 backdrop-blur-xl border border-border/50 rounded-2xl shadow-elevated z-50 animate-in fade-in slide-in-from-top-5 duration-200">
            ...
            <div className="grid grid-cols-2 gap-2 p-2">
                <button onClick={onClose} className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-secondary/50 hover:bg-secondary hover:text-primary transition-colors group">
            ...
            <div className="p-2 space-y-1">
                ...
                <button onClick={signOut} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/50 transition-colors text-sm group">
            ...
            <div className="p-2 border-t border-border/50 flex items-center justify-between text-xs text-muted-foreground">
                ...
                <button onClick={signOut} className="flex items-center gap-1.5 text-destructive hover:bg-destructive/10 transition-colors px-2 py-1.5 rounded">
